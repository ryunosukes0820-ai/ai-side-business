...
8. **テーマ・メインキーワードのロック**
   「次の記事を作成」ワークフローの開始時に決定・報告したテーマとメインキーワードは、そのワークフロー中は固定値として扱う。研究(research)の途中でClaude Codeの判断だけでテーマ・メインキーワードを変更しない。手順の詳細は「トリガーコマンド」内の該当箇所を参照する。
...
2. `topics.csv` と `articles/published/` を確認し、既存記事と重複しないテーマ・SEOキーワードを選ぶ。**この時点で決定・報告したテーマとメインキーワードは、このワークフロー実行中の固定値とする(テーマ・メインキーワードのロック)**
3. `prompts/research.md` に沿って検索意図・想定読者・事実を整理する(Web検索が必要な場合、「次の記事を作成」という指示自体を事前許可とみなし、都度確認せず進める)。一次情報が不足している場合は、以下の順番で対応する。テーマ・メインキーワードは変更せず維持したまま進めること。
   ...
   4. 再researchを行っても、記事の主要部分を支える十分な根拠が得られない場合は、**テーマを無断で変更せず、その時点でワークフローを停止する**。停止時は、固定しているテーマ・メインキーワード・調査した内容・確認できたこと・確認できなかったこと・続行する場合の選択肢をまとめて報告し、ユーザーの判断を待つ。テーマ変更が必要な場合は、必ずユーザーの承認を得てから変更する
4. `prompts/outline.md` に沿ってSEOタイトル・見出し構成・無料/有料判定・価格案・有料ライン候補を作成する
5. `prompts/writing.md` に沿って本文初稿を書く(本文の文字数は原則2,800〜3,200文字を目安にする。第7記事以前には適用しない)
6. `prompts/review.md` に沿って重複・冗長表現・AIっぽい表現を編集し、ファクトチェック要確認リストを作成する。あわせて本文の文字数を数え、2,800〜3,200文字の範囲に収まっているか確認し、範囲外であれば水増し・過度な圧縮を避けながら調整する
...
保存後、選んだテーマ・SEOタイトル・見出し構成・無料/有料判定と価格・有料ラインの位置・本文の文字数・ファクトチェック要確認リスト・ハッシュタグ案・サムネイル設計案をまとめて1回報告し、ユーザーの確認を待つ。
